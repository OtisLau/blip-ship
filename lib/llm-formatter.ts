...
function buildUserJourney(events: AnalyticsEvent[]): string {
  ...
  for (const event of events) {
    ...
    const ctx = e.elementContext as unknown as Record<string, unknown> | undefined;
    const pageCtx = e.pageContext as unknown as Record<string, unknown> | undefined;
    ...
    const scrollPos = pageCtx?.scrollPercent ? `${pageCtx.scrollPercent}% down the page` : '';
...
function analyzePatterns(events: AnalyticsEvent[]): {
  ...
  const timeSpanMs = (minTime === Infinity || maxTime === 0) ? 0 : maxTime - minTime;
  const timeSpan = timeSpanMs <= 0
    ? 'instant'
    : timeSpanMs < 1000
      ? `${timeSpanMs}ms`
      : timeSpanMs < 60000
...
function analyzeProductInteractions(events: AnalyticsEvent[]): {
  products: Array<{ name: string; price?: number; viewCount: number; addedToCart: boolean; removedFromCart: boolean }>;
  priceRange: { min: number; max: number } | null;
  actionTimeline: string[];
} {
  const productMap = new Map<string, { name: string; price?: number; views: number; addedToCart: boolean; removedFromCart: boolean }>();
  ...
  for (const event of events) {
    ...
    if (productName) {
      const existing = productMap.get(productName) || { name: productName, price: productPrice, views: 0, addedToCart: false, removedFromCart: false };

      if (event.type === 'product_view' || event.type === 'product_compare') {
        existing.views++;
        actionTimeline.push(`Viewed "${productName}"${productPrice ? ` ($${productPrice})` : ''}`);
      } else if (event.type === 'add_to_cart') {
        existing.addedToCart = true;
        actionTimeline.push(`Added "${productName}" to cart${productPrice ? ` ($${productPrice})` : ''}`);
...
function analyzeClickExpectation(events: AnalyticsEvent[]): {
  ...
  const ctx = firstEvent?.elementContext as unknown as Record<string, unknown> | undefined;
  ...
  if (ctx) {
    ...
    // Check visual cues
    if (style?.cursor === 'pointer') {
      visualCues.push('Element has cursor:pointer - universally indicates clickability');
    }
    if (tag === 'img') {
      visualCues.push('Product images are conventionally clickable for zoom/details');
    }
    if (tag === 'h3' || tag === 'h2') {
      visualCues.push('Product titles are conventionally links to product pages');
    }
    if (attrs?.src?.includes('unsplash') || attrs?.src?.includes('product')) {
      visualCues.push('Appears to be a product image - users expect to see larger version');
    }
    if (ctx.nearestInteractive) {
      visualCues.push(`Nearby clickable element: ${ctx.nearestInteractive} - user may have expected this element to be part of same interaction`);
    }

    // Determine likely intent
    if (tag === 'img') {
      likelyIntent = 'View larger image, see product details, or open product page';
      suggestedBehavior = 'Make image clickable: open lightbox, zoom, or navigate to product detail page';
      ...
      suggestedBehavior = 'Consider if this container should be a clickable card';
    } else if (tag === 'button') {
      likelyIntent = 'Trigger an action and see feedback';
      suggestedBehavior = 'Add loading state, disable during action, show success/error feedback';
    }
  }
...
function analyzeSessionBehavior(events: AnalyticsEvent[]): {
  ...
  const frustrationEvents = events.filter(e =>
    ['dead_click', 'rage_click', 'double_click', 'scroll_reversal'].includes(e.type)
  ).length;
...
export async function formatIssueForLLM(issue: UIIssue): Promise<string> {
  ...
  const elementContext = sampleEvent?.elementContext as unknown as Record<string, unknown> | undefined;
  const fullPath = elementContext?.fullPath as string | undefined;

  const component = resolveComponent(fullPath || issue.elementSelector, sampleEvent?.elementText as string | undefined);
  let componentCode = '';
  ...
  const hasMultiProductActivity = issue.patternId === 'multi_product_interaction' || productInteractions.products.length >= 2;
  const isClickFrustration = patterns.eventTypes['dead_click'] || patterns.eventTypes['rage_click'] || patterns.eventTypes['double_click'];
...
- **Types**: ${Object.entries(patterns.eventTypes).map(([t, c]) => `${c}x ${t}`).join(', ')}
...
**Products involved:**
${productInteractions.products.map(p => {
...
${patterns.allPaths.length > 1 ? `### All Affected Element Paths
...
`;
  ...
  for (const issue of issues) {
    ...
    const ctx = (issue.sampleEvents[0] as unknown as Record<string, unknown>)?.elementContext as unknown as Record<string, unknown> | undefined;
    ...
    const eventTypes = issue.sampleEvents.reduce((acc, e) => {
      acc[e.type] = (acc[e.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    const eventBreakdown = Object.entries(eventTypes).map(([t, c]) => `${c}x ${t}`).join(', ');
