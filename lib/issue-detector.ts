...
import {
  loadElementIndex,
  resolveComponentFromEvent,
  IndexedElement,
} from './element-indexer'
...
const PATTERN_RULES: PatternRule[] = [
  ...
  // Form abandonment - users starting forms but leaving them empty
  {
    ...
    fixTemplate:
      'Simplify form fields, add helpful placeholders, or break into smaller steps',
  },
  ...
  // Exit intent after specific actions
  {
    ...
    fixTemplate:
      'Consider exit-intent popup with discount, or review pricing presentation',
  },
  ...
  // SHIPPING COST REVEALED TOO LATE - exit during checkout
  {
    ...
    problemTemplate:
      'Users are abandoning checkout - possibly due to unexpected shipping costs',
    intentTemplate: 'Complete purchase but surprised by final total',
    outcomeTemplate: 'Abandoning cart at shipping/payment step',
    fixTemplate:
      'Show shipping cost estimate earlier (product page or cart), offer free shipping threshold, be transparent about costs upfront',
  },
  ...
  // CHECKOUT AUTOFILL DISABLED - slow form completion
  {
    ...
    outcomeTemplate:
      'Having to manually type everything - autofill not working',
    fixTemplate:
      'Add proper autocomplete attributes (autocomplete="name", "email", "address-line1", "postal-code", etc.)',
  },
  ...
  // ADDRESS NOT AUTOCOMPLETE - multiple focus/blur on address fields
  {
    ...
    problemTemplate:
      'Users are struggling with address entry - no autocomplete suggestions',
    ...
    fixTemplate:
      'Add Google Places Autocomplete or similar address lookup, add proper autocomplete="address-line1" attributes',
  },
  ...
  // NO MOST POPULAR TAGS - users comparing many products
  {
    ...
    problemTemplate:
      'Users are comparing many products - struggling to decide without social proof',
    intentTemplate: 'Find the best/most popular option quickly',
    outcomeTemplate:
      'No indicators of which products are bestsellers or most popular',
    fixTemplate:
      'Add "Best Seller", "Most Popular", "Customer Favorite" badges to top products, show review counts/ratings',
  },
  ...
  // NO URGENCY CUES - cart abandonment without urgency
  {
    ...
    severityThresholds: { low: 10, medium: 20, high: 35, critical: 60 },
    problemTemplate:
      'Users adding to cart but not completing purchase - no urgency to act now',
    intentTemplate: 'Decide whether to buy now or later',
    outcomeTemplate: 'Leaving to "think about it" with no reason to return',
    fixTemplate:
      'Add "Only X left in stock", "Order in next 2 hours for next-day delivery", limited-time offers, or countdown timers',
  },
  ...
  // TEXT SELECTION ON LONG CONTENT - information in paragraphs not bullets
  {
    ...
    problemTemplate:
      'Users selecting text and scrolling back - content is hard to scan',
    ...
    fixTemplate:
      'Convert product descriptions to bullet points, use icons for key features, add TL;DR summary at top',
  },
  ...
  // VISUAL HIERARCHY - dead clicks on non-primary elements
  {
    ...
    outcomeTemplate:
      'Visual hierarchy is confusing - unclear what is clickable',
    fixTemplate:
      'Make CTA buttons more prominent, increase contrast, add hover states to clickable areas, make entire card clickable',
  },
  ...
  // BUTTON NO FEEDBACK - rage/double clicks suggest missing loading state
  {
    ...
    intentTemplate:
      'User clicked button and expected immediate feedback or action',
  ...
  // IMAGE GALLERY NEEDED - dead clicks on product images suggest gallery desire
  {
    ...
    problemTemplate:
      'Users clicking/double-clicking product images expecting to enlarge',
  ...
  // COMPARISON FEATURE NEEDED - rapid product views suggest comparison shopping
  {
    ...
    severityThresholds: { low: 5, medium: 10, high: 20, critical: 40 },
    problemTemplate:
      'Users viewing multiple products rapidly - comparison shopping',
    intentTemplate:
      'Compare product features, prices, or specifications side-by-side',
    outcomeTemplate:
      'No way to compare products without opening each individually',
    fixTemplate:
      'Add compare checkbox on product cards with side-by-side comparison drawer',
  },
...
function isImageSelector(selector: string): boolean {
  return (
    selector.toLowerCase().includes('img') ||
...
export async function detectIssues(
  ...
  if (elementIndex) {
    console.log(
      `📋 [Detector] Using element index with ${elementIndex.elements.length} elements`
    )
  }
  ...
  for (const rule of PATTERN_RULES) {
    ...
    for (const [key, events] of groups) {
      ...
      const fullPath = sampleEventAny.elementContext
        ? ((sampleEventAny.elementContext as Record<string, unknown>)
            .fullPath as string)
        : undefined
      ...
      if (elementIndex) {
        ...
        if (resolved.componentPath !== 'unknown') {
          ...
          console.log(
            `🎯 [Indexer] Matched event to ${componentName} (${matchedElementId})`
          )
        }
      }
