...
/**
 * Categorize a product image as men/women/unisex using Gemini vision
 */
export async function categorizeProductImage(
  ...
  console.log(`\n🖼️ [Gemini Vision] Categorizing product: ${productName}`);
  console.log(`   Image URL: ${imageUrl.substring(0, 80)}...`);

  const prompt = `You are a fashion product categorization assistant. Analyze this clothing product and determine if it's designed for men, women, or is unisex.

Product Name: ${productName}
Image URL: ${imageUrl}

Based on the product name and typical fashion conventions, categorize this product.

Consider:
- Clothing fit and silhouette (slim vs relaxed, structured vs flowing)
- Color palette (traditionally masculine vs feminine colors)
- Style elements (patterns, cuts, details)
- Product name cues ("women's", "men's", "boyfriend fit", etc.)

For basics like hoodies, t-shirts, and joggers without specific gender cues, classify as "unisex".
...
Return ONLY valid JSON in this exact format:
{
  ...
  try {
    ...
    // Default to unisex if categorization fails
    return {
      category: 'unisex',
      confidence: 0.5,
      reasoning: 'Categorization failed, defaulting to unisex',
    };
  }
}
